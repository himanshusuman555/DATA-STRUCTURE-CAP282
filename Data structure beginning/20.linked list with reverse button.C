...
void reverse()
{
 ...
 j=len-1;
 p=q=root;
 while(i<j)       //see this in copy
 {
  k=0;
  while(k<j)        //suppose length is 5  so it will run 0 to 4
  {                 //(0<4)    condition
   q=q->link;       //(3<4) it will reach at last node in first ,(first time wen need to swap with last)
   k++;
  }
  temp=p->data;
  p->data=q->data;
  ...
  i++;
  j--;
  p=p->link;   //pointer p shift to next node feel i++ as same this
  q=root;     //q has again assined to first node
 }
 printf("revese done , enter 5 to display\n");
} 	      // nd we will travel it till the last node  again with the help
	      //of while at above till j-- is done so ,secnd last.
